...
def add_intervention(conn, intervention, start_time, end_time):
    sql = '''INSERT INTO interventions(intervention, start_time, end_time)
             VALUES(?,?,?)'''
    cur = conn.cursor()
    cur.execute(sql, (intervention, start_time, end_time))
    conn.commit()
...
def log_page():
    ...
    with st.form("intervention_form"):
        ...
        if submitted:
            ...
            add_intervention(conn, intervention, start_time.strftime("%H:%M"), end_time.strftime("%H:%M"))
